...
function setGlobalCohorts(req, res, next) {
    ...
    var seniorNumber = +req.query.junior_cohort + 1 < 10 ? `0${+req.query.junior_cohort + 1}` : `${+req.query.junior_cohort + 1}`
    globals.JUNIOR_COHORT = 'GHRBLD' + juniorNumber
    globals.SENIOR_COHORT = 'GHRBLD' + seniorNumber
    next()
}
...
function sendCsvToCalendar(req, res, next) {
    var juniorData = req.calendarData.junior
    var seniorData = req.calendarData.senior
    ...
    Promise.all([
        calendar.calendars.insert({
            resource: {
                summary: globals.JUNIOR_COHORT + 'TEST'
            },
        }).then(cal => {
            var id = cal.data.id
            return Promise.all(juniorData.map(event => {
                return calendar.events.insert({
                    calendarId: id,
                    resource: event
                }).then(x => x).catch(e => next(e))
            }))
        }).catch(e => next(e)),

        calendar.calendars.insert({
            resource: {
                summary: globals.SENIOR_COHORT + 'TEST'
            },
        }).then(cal => {
            var id = cal.data.id
            return Promise.all(seniorData.map(event => {
                return calendar.events.insert({
                    calendarId: id,
                    resource: event
                }).then(x => x).catch(e => next(e))
    ...
    res.status(200).send(`Your calendars are being made, please check your account for calendars called ${globals.JUNIOR_COHORT} and ${globals.SENIOR_COHORT}`)
}
