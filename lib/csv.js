...
function setSubject(row, record) {
    row.event['summary'] = `${record['Type']}: ${record['Lecture / Activity / Exercise']}`
    row.csv['Subject'] = `${record['Type']}: ${record['Lecture / Activity / Exercise']}`
}
            ...
            return new Promise(function (resolve, reject) {
                stringify(sheetData, function (err, data) {
                    ...
                    parse(data, parseOptions).on('readable', function () {
                        ...
                        stringify(convertedData.senior.map(row => row.csv), stringifyOptions, function (err, seniorData) {
                            if (err) return reject(err);
                            stringify(convertedData.junior.map(row => row.csv), stringifyOptions, function (err, juniorData) {
                                if (err) return reject(err)
                                return resolve({
                                    csv: {
                                        senior: seniorData,
